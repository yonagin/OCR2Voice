...
def get_path_config() -> PathConfig:
    path_config_path = Path("tts_configs/paths.yml")
    if not path_config_path.exists():
        shutil.copy(src="tts_configs/default_paths.yml", dst=path_config_path)
        logger.info(
            f"A configuration file {path_config_path} has been generated based on the default configuration file default_paths.yml."
        )
        logger.info(
            "Please do not modify tts_configs/default_paths.yml. Instead, modify tts_configs/paths.yml."
        )
...
def get_config() -> Config:
    ...
    try:
        config = Config("tts_config.yml", path_config)
    except (TypeError, KeyError):
        logger.warning("Old tts_config.yml found. Replace it with tts_default_config.yml.")
        shutil.copy(src="default_tts_config.yml", dst="config.yml")
